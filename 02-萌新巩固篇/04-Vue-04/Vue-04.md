...
* 脚手架  
    * 安装`npm i -g @vue/cli` 或 `yarn global add @vue/cli`
    * 创建项目的两种方式
        * 命令行 - `vue create 项目名称`
        * UI - `vue ui`                              
...
6. 脚手架
    * 安装好脚手架，其实可以直接敲个vue，就能看到些提示 
        
        ![](./images/脚手架提示信息.jpg)

    * 那我们根据提示，敲个`vue create app`  
        * 提示信息其实很多，一开始会让你选择默认还是手动选择安装
        * 如果选择手动安装，会提示你空格选择，a全向，i反向选择  
        * 这边我们选择默认的安装下看下效果，然后use yarn 
        * 装完以后有提示信息，`cd app` + `yarn serve`
        * 然后就能看到应用搭好了
    ...
    * 我们还可以通过ui去创建项目
        ...
        * 点击tab页创建，选择路径，然后点击**在此创建新项目**

            ![](./images/在此创建新项目.jpg)  

        * 然后就是傻瓜式的操作，输入项目名字，需要什么就点什么  

            ![](./images/需要什么点什么.jpg)     

        * 创建完项目就会看到

            ![](./images/创建完项目就能看到.jpg)   

        * 可以在任务中找到运行

            ![](./images/任务运行.jpg)

        * 然后点击启动app即可~就把所有命令的操作变成了可视化操作           
